...
app.route("/isincart/:emailUser").get(async function (req, res) {
...
app
    ...
    connectMysql.query(
      ...
      function (err, result) {
        ...
        if (result.length > 0) {
          ...
          connectMysql.query(
            "INSERT INTO `carts` (email,code,quantity) VALUES ('" +
              req.params.emailUser +
              "'," +
              req.body.code +
              "," +
  ...
  .get(async function (req, res) {
    connectMysql.query(
      `SELECT * FROM carts INNER JOIN products
  ...
  .delete(async function (req, res) {
    ...
    connectMysql.query(
      `DELETE FROM carts WHERE email='${email}' AND code=${code};`,
      function (err, result) {
        if (err) throw err;
...
app
  .route("/wishlist/:emailUser")
  .post(async function (req, res) {
    connectMysql.query(
      "SELECT * FROM `wishlists` WHERE email ='" +
        req.params.emailUser +
        "' AND code =" +
        req.body.code +
        ";",
      function (err, result) {
        ...
        if (result.length > 0) {
          ...
          connectMysql.query(
            "INSERT INTO `wishlists` (email,code) VALUES ('" +
              req.params.emailUser +
              "'," +
              req.body.code +
              ")",
  ...
  .get(async function (req, res) {
    connectMysql.query(
      `SELECT * FROM wishlists INNER JOIN products
  ...
  .delete(async function (req, res) {
    ...
    connectMysql.query(
      `DELETE FROM wishlists WHERE email='${email}' AND code=${code};`,
      function (err, result) {
        if (err) throw err;
...
app
  ...
  .patch(async function (req, res) {
    connectMysql.query(
      `SELECT * FROM users WHERE email ='${req.params.emailUser}';`,
      function (err, result) {
        if (err) throw err;
        if (result.length > 0) {
...
app.route("/productsFilter").post(async function (req, res) {
  ...
  connectMysql.query(
    ...
    function (err, result) {
      result = result.filter((item) => {
        return (
          checkArrayInArray(item.colors.split(","), filter.color) &&
          checkArrayInArray(item.size.split(","), filter.size)
        );
      });
      if (err) throw err;
      const startIndex =
        (parseInt(req.body.pageNumber) - 1) * parseInt(req.body.pageSize);
      res.send(
        result.slice(startIndex, startIndex + parseInt(req.body.pageSize))
      );
    }
  );
...
app.route("/product/:idProduct").get(async function (req, res) {
  connectMysql.query(
    `SELECT * FROM products WHERE code ='${req.params.idProduct}';`,
    function (err, result) {
      if (err) throw err;
      if (result.length > 0) {
        res.send(result[0]);
